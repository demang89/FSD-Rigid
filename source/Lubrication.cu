...
__device__ __constant__ int NREG = 0;
__device__ __constant__ double D_MIN = 0.000100;
...
__global__ void check_Lubrication_interactions(
					const Scalar4 *d_pos,
					int *d_body_tag,
  ...
  if ( tidx < group_size ) {
    ...
    for (unsigned int neigh_idx = 0; neigh_idx < n_neigh; neigh_idx++) {
      ...
      // Early exit if lubrication already found globally or in block
      if (*d_has_lubrication || block_found) break;

      // Get the current neighbor index
      unsigned int curr_neigh = d_nlist[ head_idx + neigh_idx ];
	  //Deepak:included it for rigid body simulations
    ...
    // If any thread in block found lubrication, update global flag
    if (threadIdx.x == 0 && block_found) {
        atomicExch(d_has_lubrication, 1);
        __threadfence(); // ensure visibility to other blocks
    }
...
__global__ void Lubrication_RFU_kernel(
  ...
  if ( tidx < group_size ) {
    ...
    Scalar4 posi = d_pos[ curr_particle ];
    Scalar3 ui, wi, uj, wj;
    ui.x = d_Velocity[ 6*tidx     ];
    ui.y = d_Velocity[ 6*tidx + 1 ];
    ui.z = d_Velocity[ 6*tidx + 2 ];
    wi.x = d_Velocity[ 6*tidx + 3 ];
    wi.y = d_Velocity[ 6*tidx + 4 ];
    wi.z = d_Velocity[ 6*tidx + 5 ];
    ...
    for (unsigned int neigh_idx = 0; neigh_idx < n_neigh; neigh_idx++) {
	  ...
	  //Deepak:included it for rigid body simulations
      if((d_body_tag[curr_neigh]==-1)||(d_body_tag[curr_neigh]==d_body_tag[curr_particle])) continue;
	  int curr_local_index = d_local_index[curr_neigh];

      // Position of neighbor particle
      Scalar4 posj = d_pos[ curr_neigh ];

      // (disturbance) Velocity of neighbor particle
	  //Deepak : modified index from curr_neigh to curr_local_index
      uj.x = d_Velocity[ 6*curr_local_index ];
      uj.y = d_Velocity[ 6*curr_local_index + 1 ];
      uj.z = d_Velocity[ 6*curr_local_index + 2 ];
      wj.x = d_Velocity[ 6*curr_local_index + 3 ];
      wj.y = d_Velocity[ 6*curr_local_index + 4 ];
      wj.z = d_Velocity[ 6*curr_local_index + 5 ];
      ...
      if ( (distSqr < rlubsq) && (distSqr > 0.0) ){	
		...
		if ( dist <= ( 2.0+D_MIN ) ){
			...
			int ind = log10( ( dist - 2.0 ) /  ResTable_min ) / ResTable_dr;
    ...
    // Write to output
	//Deepak :  modified from curr_particle to tidx for rigid body
    d_Force[ 6*tidx     ] = fi.x;
    d_Force[ 6*tidx + 1 ] = fi.y;
    d_Force[ 6*tidx + 2 ] = fi.z;
    d_Force[ 6*tidx + 3 ] = li.x;
    d_Force[ 6*tidx + 4 ] = li.y;
    d_Force[ 6*tidx + 5 ] = li.z;
...
__global__ void Lubrication_RFE_kernel(
  ...
  if ( tidx < group_size ) {
	...
	// Write to output
	//Deepak :  modified index from curr_particle to tidx for rigid body
	d_Force[ 6*tidx     ] += fi.x; 
	d_Force[ 6*tidx + 1 ] += fi.y; 
	d_Force[ 6*tidx + 2 ] += fi.z; 
	d_Force[ 6*tidx + 3 ] += li.x; 
...
__global__ void Lubrication_RSU_kernel(
  ...
  if ( tidx < group_size ) {
	...
	// Deepak: changed index from curr_particle to tidx
    Scalar4 posi = d_pos[ curr_particle ];
    Scalar3 ui, wi, uj, wj;
    ui.x = d_Velocity[ 6*tidx ];
    ui.y = d_Velocity[ 6*tidx + 1 ];
    ...
    for (neigh_idx = 0; neigh_idx < n_neigh; neigh_idx++) {
      ...
      // Get the current neighbor index
      curr_neigh = d_nlist[ head_idx + neigh_idx ];
	  //Deepak:added for rigid simulations
      if((d_body_tag[curr_neigh]==-1)||(d_body_tag[curr_neigh]==d_body_tag[curr_particle])) continue;
	  int curr_local_index = d_local_index[curr_neigh];
	  ...
	  //Deepak:changed index from curr_neigh to curr_local_index
	...
	//Deepak changed index from curr_particle to tidx
    d_Stresslet[ 5*tidx ]     -= si[0];
    d_Stresslet[ 5*tidx + 1 ] -= si[1];
    d_Stresslet[ 5*tidx + 2 ] -= si[2];
    d_Stresslet[ 5*tidx + 3 ] -= si[3];
    d_Stresslet[ 5*tidx + 4 ] -= si[4];
...
__global__ void Lubrication_RSE_kernel(
  ...
  if ( tidx < group_size ) {
    ...
    for (neigh_idx = 0; neigh_idx < n_neigh; neigh_idx++) {
		...
		//Deepak : added for rigid body simulations
		if((d_body_tag[curr_neigh]==-1)||(d_body_tag[curr_neigh]==d_body_tag[curr_particle])) continue;
...
__global__ void Lubrication_RSEgeneral_kernel(
	...
	if ( tidx < group_size ) {
		...
		Scalar4 posi = d_pos[ curr_particle ];
		Scalar Ei[5];
		Ei[0] = d_Strain[  5*tidx + 0 ];
		Ei[1] = d_Strain[  5*tidx + 1 ];
		Ei[2] = d_Strain[  5*tidx + 2 ];
		Ei[3] = d_Strain[  5*tidx + 3 ];
		Ei[4] = d_Strain[  5*tidx + 4 ];
		...
		for (neigh_idx = 0; neigh_idx < n_neigh; neigh_idx++) {
			...
			Scalar4 posj = d_pos[ curr_neigh ];
			Scalar Ej[5];
			Ej[0] = d_Strain[ 5*curr_local_index ];
			Ej[1] = d_Strain[ 5*curr_local_index + 1 ];
			Ej[2] = d_Strain[ 5*curr_local_index + 2 ];
			Ej[3] = d_Strain[ 5*curr_local_index + 3 ];
			Ej[4] = d_Strain[ 5*curr_local_index + 4 ];
