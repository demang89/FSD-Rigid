...
//Structure for tangential contact spring for a pair
struct ContactSlot {
    unsigned int neighbor_id;      // Holds SLOT_EMPTY_KEY or particle index
    uint64_t last_seen_frame;  // Tracks when this contact was last active
    Scalar xi[3];                  // Friction spring state vector
};
...
struct BrownianData
{
	...
	uint64_t timestep;	//!< Simulation time step (used by RNG)
...
struct MobilityData
{
	...
	size_t *headlist;		//!< Ewald sum real space headlist
...
struct ResistanceData
{
	...
	size_t *headlist;		//!< Lubrication interaction headlist
	...
	Scalar *table_dist;	//!< Resistance tabulation distances
	Scalar *table_vals; 	//!< Resistance tabulation values
...
struct WorkData
{
	...
	//Scalar *dot_sum; 	//!< Partial dot product sum
    	Scalar *bro_gauss;   //zhoge: Gaussian random variables
