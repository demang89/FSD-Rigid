...
private:
    double getCurrentOmega(unsigned int timestep) {
        return m_omega_0 * exp( m_dt * (timestep - m_offset) * log(m_omega_f / m_omega_0) / m_periodT );
    }
    double getCurrentPhase(unsigned int timestep) {
        return m_periodT * m_omega_0 / log( m_omega_f / m_omega_0 ) * ( exp( m_dt * (timestep - m_offset) * log(m_omega_f / m_omega_0) / m_periodT ) - 1.0 );
    }
