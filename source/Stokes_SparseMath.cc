...
void Stokes::setSparseMath(){
	...
	GPUArray<unsigned int> n_nneigh_pruned( group_size_b, m_exec_conf );
	GPUArray<unsigned int> n_headlist_pruned( group_size_b + 1, m_exec_conf );
	GPUArray<unsigned int> n_nlist_pruned( (max_neigh+1)*group_size_b, m_exec_conf );

	m_nneigh_pruned.swap( n_nneigh_pruned );
	m_headlist_pruned.swap( n_headlist_pruned );
	m_nlist_pruned.swap( n_nlist_pruned );

	//Deepak:added for rigid body matrix
	m_nnz =  ( 36 * ( max_neigh + 1 ) ) * group_size_b;
	//m_nnz =  36 * group_size_b;
	
	GPUArray<int>   n_L_RowInd( m_nnz, m_exec_conf );
	GPUArray<int>   n_L_RowPtr( 6*group_size_b+1, m_exec_conf );
	GPUArray<int>   n_L_ColInd( m_nnz, m_exec_conf );
	GPUArray<Scalar> n_L_Val(    m_nnz, m_exec_conf );
	...
	GPUArray<int>   n_HasNeigh( group_size_b, m_exec_conf );	//!< Whether a particle has neighbors or not
	GPUArray<Scalar> n_Diag( 6*group_size_b, m_exec_conf );	//!< Diagonal preconditioner elements 
	m_Diag.swap( n_Diag );
	m_HasNeigh.swap( n_HasNeigh );

	// Index arrays needed to construct sparse matrices
	GPUArray<unsigned int> n_nneigh_less( group_size_b, m_exec_conf ); //!< Number of neighbors with index less than particle ID
	GPUArray<unsigned int> n_NEPP( 2*group_size_b, m_exec_conf );       //!< Number of non-zero entries per particle in sparse matrices
	GPUArray<unsigned int> n_offset( (group_size_b+1), m_exec_conf );   //!< Particle offset into sparse matrix arrays
	...
	//
	GPUArray<Scalar> n_Scratch1( 6*group_size_b, m_exec_conf );		//!< Scratch storage for re-ordered matrix-vector multiplication 
	GPUArray<Scalar> n_Scratch2( 11*group_size+6*group_size_b, m_exec_conf );	//!< Scratch storage for saddle point preconditioning
	GPUArray<Scalar> n_Scratch3( m_nnz, m_exec_conf );	//!< Scratch Storage for Value reordering
        GPUArray<Scalar> n_Scratch4( 6*group_size, m_exec_conf );
	GPUArray<Scalar> n_Scratch5( 6*group_size, m_exec_conf );
	GPUArray<Scalar> n_Scratch6( 6*group_size, m_exec_conf );
	GPUArray<Scalar> n_Scratch7( 6*group_size_b, m_exec_conf );
	GPUArray<Scalar> n_Scratch8( 6*group_size, m_exec_conf );
	GPUArray<Scalar> n_EinfForce( 6*group_size, m_exec_conf );	
	GPUArray<int>   n_prcm( 6*group_size_b, m_exec_conf );		//!< matrix re-ordering vector using Reverse-Cuthill-Mckee (RCM)

	m_Scratch1.swap( n_Scratch1 );
	m_Scratch2.swap( n_Scratch2 );
	m_Scratch3.swap( n_Scratch3 );
	m_Scratch4.swap( n_Scratch4 );
	m_Scratch5.swap( n_Scratch5 );
	m_Scratch6.swap( n_Scratch6 );
	m_Scratch7.swap( n_Scratch7 );
	m_Scratch8.swap( n_Scratch8 );
	m_EinfForce.swap( n_EinfForce );
	m_prcm.swap( n_prcm );
