...
void Debug_Lattice_SpinViscosity( 
	...
	int numel = 6 * group_size_b;

	// Initialize input and output vector 
	Scalar *h_x, *d_x;
	h_x = (Scalar *)malloc( (11*group_size+6*group_size_b)*sizeof(Scalar) );
	cudaMalloc( (void**)&d_x, (11*group_size+6*group_size_b)*sizeof(Scalar) );
	
	Scalar *h_y, *d_y;
	h_y = (Scalar *)malloc( numel*sizeof(Scalar) );
	cudaMalloc( (void**)&d_y, (11*group_size+6*group_size_b)*sizeof(Scalar) );

	for ( int ii = 0; ii < (11*group_size+6*group_size_b); ++ii ){
		h_x[ii] = 0.0;
	}
	for ( int ii = 0; ii < group_size_b; ++ii ){
		h_x[ 11*group_size + 6*ii + 4 ] = 1.0;
	}

	cudaMemcpy( d_x, h_x, (11*group_size+6*group_size_b)*sizeof(Scalar), cudaMemcpyHostToDevice );
	...
	//
	Scalar3 L = box.getL();
	Scalar phi = 4 * 3.1415926536 / 3 * Scalar(group_size_b) / (L.x*L.y*L.z);
	Scalar spin_viscosity = 0.0;
        for ( int ii = 0; ii < group_size_b; ++ii ){
                spin_viscosity += ( 6.0 * phi ) * ( 3.0 / 4.0 ) * (-1.0 / h_y[ 6*ii + 4 ] );
        }
        spin_viscosity /= Scalar( group_size_b );
	printf( "\n\nSPINVISCOSITY: %8.6f\n\n\n", spin_viscosity );
...
__global__ void AddStrainRate_kernel(
                                     Scalar *d_b,
                                     Scalar *Einf,
                                     Scalar4 *d_pos,
                                     unsigned int *d_group_members,
        ...
        if ( tidx < group_size ) {
                ...
                d_b[ 6 * tidx + 0 ] = (Einf[0] * d_rel_pos[idx].x + Einf[1] * d_rel_pos[idx].y + Einf[2] * d_rel_pos[idx].z);
                d_b[ 6 * tidx + 1 ] = (Einf[1] * d_rel_pos[idx].x + Einf[4] * d_rel_pos[idx].y + Einf[3] * d_rel_pos[idx].z);
				d_b[ 6 * tidx + 2 ] = (Einf[2] * d_rel_pos[idx].x + Einf[3] * d_rel_pos[idx].y -(Einf[0]+Einf[4]) * d_rel_pos[idx].z);

                // Index into array
                unsigned int ind = 6*group_size + 5*tidx;

                // Add ambient strain rate Einf (E_xy = E_yx = shear_rate/2, all else 0)
                d_b[ ind + 0 ] = 2*Einf[0]+Einf[4];        // E_xx - E_zz
                d_b[ ind + 1 ] = 2*Einf[1]; // E_xy * 2
                d_b[ ind + 2 ] = 2*Einf[2];        // E_xz * 2
                d_b[ ind + 3 ] = 2*Einf[3];        // E_yz * 2
                d_b[ ind + 4 ] = 2*Einf[4]+Einf[0];        // E_yy - E_zz
	}
}
...
void Debug_Lattice_ShearViscosity( 
	...
	Scalar *h_x, *d_x;
	h_x = (Scalar *)malloc( (11*group_size+6*group_size_b)*sizeof(Scalar) );
	cudaMalloc( (Scalar**)&d_x, (11*group_size+6*group_size_b)*sizeof(Scalar) );
	cudaMemset(d_x, 0, (11*group_size+6*group_size_b) * sizeof(Scalar));
	
        Scalar *d_E;
        cudaMalloc( (Scalar**)&d_E, 5*sizeof(Scalar) );
        cudaMemset(d_E, 0, 5* sizeof(Scalar));
	...
	unsigned int *h_group_members;
	h_group_members = (unsigned int *)malloc( (group_size)*sizeof(unsigned int) );
	cudaMemcpy( h_group_members, d_group_members, (group_size)*sizeof(unsigned int), cudaMemcpyDeviceToHost );

        int *h_body_tag;
        h_body_tag = (int *)malloc( (group_size+group_size_b)*sizeof(int) );
        cudaMemcpy( h_body_tag, res_data->body_tag, (group_size+group_size_b)*sizeof(int), cudaMemcpyDeviceToHost );
	...
	for ( int jj = 0; jj < 2; ++jj ){
		...
		Scalar E[5] = {0.0};
		E[ jj ] = 1.0;
		cudaMemcpy( d_E, E, 5*sizeof(Scalar), cudaMemcpyHostToDevice );
		AddStrainRate_kernel<<< grid, threads >>>( d_x,
                                           		d_E,
                                           		d_pos,
                                           		d_group_members,
                                           		group_size,
		...
		Scalar3 L = box.getL();
		Scalar phi = 4 * 3.1415926536 / 3 * Scalar(group_size_b) / (L.x*L.y*L.z);
		Scalar shear_viscosity[group_size_b] = {0.0};
		for ( int ii = 0; ii < group_size; ii++ ){
			int idx = h_group_members[ii];
			int bidx = h_body_tag[idx];
			shear_viscosity[bidx] += (h_y[ 5*ii + jj ]) ;
		}
                printf("stress on 1 and 2 are %f and %f\n",shear_viscosity[0],shear_viscosity[1]); 

		Scalar shear_viscosity_avg = 0.0;
		for ( int ii = 0; ii < group_size_b; ii++ ){
			shear_viscosity_avg += shear_viscosity[ii];
		}
		shear_viscosity_avg /= Scalar( group_size_b );

		if ( jj == 0 ){
			alpha = 1 + 2.5 * phi * shear_viscosity_avg;
		}
		else if ( jj == 1 ){
			beta = 1 + 2.5 * phi * shear_viscosity_avg;
		}
...
/*   void Debug_Random_Dss2( 
	...
	unsigned int *d_tag;
	cudaMalloc( (void**)&d_tag, N*sizeof(unsigned int) );
	cudaMemset(d_tag, 0, N * sizeof(unsigned int));
   	...
   	float *h_y, *d_y;
   	h_y = (float *)malloc( (11*N)*sizeof(float) );
   	cudaMalloc( (void**)&d_y, (11*N)*sizeof(float) );
   
   	for ( int ii = 0; ii < 6*N; ++ii ){
   		h_x[ii] = 0.0f;
   	}
   
        for ( int ii = 0; ii < 11*N; ++ii ){
                h_y[ii] = 0.0f;
        }

   	// Copy to device
   	cudaMemcpy( d_x, h_x, 6*N*sizeof(float), cudaMemcpyHostToDevice );
   	cudaMemcpy( d_y, h_y, 11*N*sizeof(float), cudaMemcpyHostToDevice );
   	...
   	for ( int nn = 0; nn < nrepeats; ++nn ){
   		...
   		( bro_data->seed_rfd1   ) = rand();
		( bro_data->seed_rfd2   ) = rand();
                // printf("random seeds %12i %12i %12i %12i\n",bro_data->seed_ff_rs,
				// bro_data->seed_ff_ws,bro_data->seed_nf,bro_data->seed_rfd);

   		// Compute the velocity
		Integrator_ComputeVelocity(0, 
					0,
					d_x,		// input
					d_y,      //output (FSD velocity and stresslet, 11N)
					dt,
					0.0,		//shear rate
   		...
   		// Copy to host
   		cudaMemcpy( h_y, d_y, 11*N*sizeof(float), cudaMemcpyDeviceToHost );
   
   		// Compute the dot product
   		for ( int ii = 0; ii < N; ++ii ){
   
   			Dss += ( h_y[ 11*ii + 0 ] * h_y[ 11*ii + 0 ] ) * ( dt / ( 3.0 * float(N) * 2.0 ) );
   			Dss += ( h_y[ 11*ii + 1 ] * h_y[ 11*ii + 1 ] ) * ( dt / ( 3.0 * float(N) * 2.0 ) );
   			Dss += ( h_y[ 11*ii + 2 ] * h_y[ 11*ii + 2 ] ) * ( dt / ( 3.0 * float(N) * 2.0 ) );
...
void Debug_Lattice_ExtensionalViscosity(
					MobilityData *mob_data,
					ResistanceData *res_data,
					KernelData *ker_data,
					WorkData *work_data,
	...
	Scalar E[5] = {1.0,0.0,0.0,0.0,-0.5}; //strain tensor for axial extensional flow
	cudaMemcpy( d_E, E, 5*sizeof(Scalar), cudaMemcpyHostToDevice );
	AddStrainRate_kernel<<< grid, threads >>>( d_x,
											d_E,
											d_pos,
	...
	// Solve for far-field stress and velocity
	Solvers_Saddle(
			d_x,  // input
			d_y,  // output
			d_pos,
	...
	//Copy stress to host device
	cudaMemcpy( h_y, &d_y[6*group_size], 5*group_size*sizeof(Scalar), cudaMemcpyDeviceToHost );

        //Compute average xx and yy components of stress
        Scalar3 L = box.getL();
        Scalar ar = Scalar(group_size)/Scalar(group_size_b);
        //Scalar phi = 3. / 4. * 3.14 * ar / (L.x*L.y*L.z);
        Scalar nl3 = Scalar(group_size_b)*pow(ar,3)/(L.x*L.y*L.z);

        Scalar shear_viscosity[2] = {0.0};
        for ( int ii = 0; ii < group_size; ii++ ){
                shear_viscosity[0] += h_y[ 5*ii ];
                shear_viscosity[1] += h_y[ 5*ii +4 ];
        }
        shear_viscosity[0] /= Scalar(group_size_b);
        shear_viscosity[1] /= Scalar(group_size_b);
        printf("%f,%f,%f\n",nl3,shear_viscosity[0],shear_viscosity[1]);
