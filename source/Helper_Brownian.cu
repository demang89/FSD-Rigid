...
__global__ void Brownian_LanczosMatrixMultiply_kernel(
	...
	if (idx < group_size) {
		...
		for ( int jj = 0; jj < numel; ++jj ){
			d_b[ numel*idx + jj ] = 0.0;
		}
		...
		for ( int ii = 0; ii < m; ++ii ){
			...
			Scalar xcurr = d_x[ii];
			int tid = ii*numel*group_size + numel*idx;
			for ( int jj = 0; jj < numel; ++jj ){
				d_b[ numel*idx + jj ] += d_A[ tid + jj ] * xcurr;
			}
		}
...
__global__ void Brownian_Farfield_LinearCombinationFTS_kernel(
	...
	if (group_idx < group_size) {
		...
		Scalar4 A4 = d_a[idx];
		Scalar4 B4 = d_b[idx];
		d_c[idx] = make_scalar4( coeff_a*A4.x + coeff_b*B4.x, coeff_a*A4.y + coeff_b*B4.y, 
					coeff_a*A4.z + coeff_b*B4.z, d_c[idx].w);

		A4 = d_a[ group_size + 2*idx ];
		B4 = d_b[ group_size + 2*idx ];
		d_c[ group_size + 2*idx ] = make_scalar4( coeff_a*A4.x + coeff_b*B4.x, coeff_a*A4.y + coeff_b*B4.y, 
						coeff_a*A4.z + coeff_b*B4.z, coeff_a*A4.w + coeff_b*B4.w );
		
		A4 = d_a[ group_size + 2*idx+1 ];
		B4 = d_b[ group_size + 2*idx+1 ];
		d_c[ group_size + 2*idx+1 ] = make_scalar4( coeff_a*A4.x + coeff_b*B4.x, coeff_a*A4.y + coeff_b*B4.y, 
						coeff_a*A4.z + coeff_b*B4.z, coeff_a*A4.w + coeff_b*B4.w );
	}
}
...
void Sqrt_multiply( Scalar *d_V,       //input
		    ...
		    WorkData *work_data )
{
  ...
  int INFO = LAPACKE_dstedc( LAPACK_ROW_MAJOR,  
			     'I',               //compute eigenvectors of tridiagonal matrix
			     m,                 //size of Tm
			     h_alpha1,          //diagonals of Tm (input) and ascending eigenvalues of Tm (output)
			     h_beta1,       //sub-diagonals of Tm (input), referenced from [1] because [0] is a placeholder (destroyed at output)
			     Q,                 //output: orthonormal eigenvectors of Tm
			     m );               //leading dimension of Q

  for ( int i = 0; i < m; ++i ){
    if(fabs(h_alpha1[i])<1e-7) h_alpha1[i]=1e-7;
  }
  ...
  for ( int i = 0; i < m; ++i ){
    h_alpha1[i] = sqrt(h_alpha1[i]) * Q[i];  //the Q index is continuous because of row-major in stedc
  }
  ...
  Brownian_LanczosMatrixMultiply_kernel<<<grid,threads>>>( 
		  					d_V,   //input (basis vectors)
