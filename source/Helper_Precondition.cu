	...
	if ( tid < group_size ){
		...
		for ( int ii = 0; ii < 6; ++ ii ){
			...
			for ( int col_ind = start; col_ind < end; col_ind++ ){
				...
				// are 4/3
				if ( col == row ){
					d_L_Val[ col_ind ] += (ichol_relaxer * ( ( ii < 3 ) ? 1.0f : 1.33333333f));
					//d_L_Val[ col_ind ] += (d_L_Val[ col_ind ]*ichol_relaxer + ichol_relaxer * ( ( ii < 3 ) ? 1.0f : 1.33333333f));
					break;
				}
...
/*
Deepak : Compute sigma operator matrix in csr format 
	from relative primary particle positions wrt CoM
	row_ptr 	(output)    CSR row pointer for sigma operator
	col_index 	(output)    CSR col pointer for sigma operator
	values	 	(output)    CSR values pointer for sigma operator
	body_tag	(input)		Aggregate tag id pointer for particles
	rel_pos		(input)		relative position pointer for particles
	group_size	(input)		Number of primary particles
	d_group_members	(input)	Primary particle indices 
*/
__global__ void buildTransformationCSR(
					int* row_ptr, 
					int* col_index, 
					Scalar* values, 
					int* body_tag,
	...
	{
		int row_start = 6 * tidx;
		int idx = d_group_members[tidx];
		int body_id = body_tag[idx];
		Scalar3 r = make_scalar3(rel_pos[idx].x, rel_pos[idx].y, rel_pos[idx].z);
		int col_base = 6 * body_id;

		int write_idx = 12 * tidx; // 6 non-zeros per row, 6 rows → max 36 per particle

		// Row 0 (vx)
		row_ptr[row_start + 1] = write_idx + 3;
		col_index[write_idx + 0] = col_base + 0; values[write_idx + 0] = 1.0f;
		col_index[write_idx + 1] = col_base + 4; values[write_idx + 1] =  r.z;
		col_index[write_idx + 2] = col_base + 5; values[write_idx + 2] = -r.y;

		// Row 1 (vy)
		row_ptr[row_start + 2] = write_idx + 6;
		col_index[write_idx + 3] = col_base + 1; values[write_idx + 3] = 1.0f;
		col_index[write_idx + 4] = col_base + 3; values[write_idx + 4] = -r.z;
		col_index[write_idx + 5] = col_base + 5; values[write_idx + 5] =  r.x;

		// Row 2 (vz)
		row_ptr[row_start + 3] = write_idx + 9;
		col_index[write_idx + 6] = col_base + 2; values[write_idx + 6] = 1.0f;
		col_index[write_idx + 7] = col_base + 3; values[write_idx + 7] =  r.y;
		col_index[write_idx + 8] = col_base + 4; values[write_idx + 8] = -r.x;

		// Row 3 (omega_x)
		row_ptr[row_start + 4] = write_idx + 10;
		col_index[write_idx + 9]  = col_base + 3; values[write_idx + 9]  = 1.0f;

		// Row 4 (omega_y)
		row_ptr[row_start + 5] = write_idx + 11;
		col_index[write_idx + 10] = col_base + 4; values[write_idx + 10] = 1.0f;

		// Row 5 (omega_z)
		row_ptr[row_start + 6] = write_idx + 12;
		col_index[write_idx + 11] = col_base + 5; values[write_idx + 11] = 1.0f;
	}
}
